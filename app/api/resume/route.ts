...
[예시 3 — 직무 역량]
수료율 관리가 숫자만의 문제가 아니라는 걸 알게 된 건 한 수강생의 전화 한 통 때문이었다. 진도가 멈춰있어서 연락을 드렸더니, 야간 근무 중이라 수강 시간이 없다고 했다. 수료 기한을 확인하고, 가능한 구간을 찾아서 짧은 단위로 나눈 학습 일정을 문자로 드렸다. 일주일 뒤 수료했다. 그 이후 나는 진도 미달 수강생에게 연락할 때 항상 상황부터 물었다. 스프레드시트로 수강생별 진도율과 최종 연락 일자를 관리했고, 주 1회 현황을 팀장에게 보고했다. 개인적인 친절이 아니라 관리 체계가 수료율을 올린다.
...
위 예시들의 공통점:
1. 첫 문장이 숫자 또는 구체적 장면으로 시작한다 ("마감 이틀 전이었다", "처음 3주는 완전히 틀렸다")
