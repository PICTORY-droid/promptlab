...
import {
  scanPromptAction,
  type SafeCheckActionState,
} from "@/app/(app)/safecheck/actions";
...
function getLevelDescription(level: string) {
  if (level === "block") {
    return "저장 전 본문 수정이 필요합니다.";
  }

  if (level === "review") {
    return "비공개 초안 저장을 권장합니다.";
  }
...
export default function WriteSafeCheckPanel() {
  ...
  return (
    <form action={formAction} className="space-y-3">
      ...
      {!result ? null : (
        <div className={isDetailOpen ? "space-y-3" : "hidden"}>
          {result.findings.length === 0 ? (
            <div className="rounded-2xl bg-emerald-50 p-4">
              ...
              <p className="mt-1 text-sm leading-6 text-emerald-800">
                저장 전 실제 개인정보나 내부자료가 없는지 한 번 더 확인하세요.
              </p>
            </div>
          ) : (
