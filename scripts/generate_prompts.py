...
def main():
    ...
    while generated < 50 and attempts < max_attempts:
        ...
        prompt_data = generate_prompt(client, combo, existing_keywords, existing_authors, created_at)
...
def pick_date(used_dates, today):
    ...
    chosen = random.choice(all_dates) if all_dates else today.strftime("%Y-%m-%d")
...
# Gemini로 프롬프트 생성
# ==========================================
def generate_prompt(client, combo, existing_keywords, existing_authors, created_at):
...
## 작성 규칙
- 제목: 구체적이고 실용적 (예: "간호사 교대 스케줄 자동화 — Python으로 최적화")
- 설명: 2~3문장 핵심 요약
- 내용: 프롬프트 섹션 4개 이상, 각 150자 이상, 변수 4개 이상
- 작성자명: 한국어 5글자 이하 또는 영어 SNS 스타일
